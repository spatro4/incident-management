import { GraduationCap, School, LogOut, Home } from 'lucide-react'
import { useApp } from '../../context/AppContext'

export default function Header() {
  const { role, setRole, isLoggedIn, state, logOut, goHome } = useApp()
      ...
      <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={goHome}
            title="Home"
            className="flex items-center gap-2 p-1.5 -m-1.5 rounded-xl hover:bg-slate-100 transition-colors"
          >
            <span className="text-2xl">🧮</span>
            <span className="font-display text-lg font-extrabold text-slate-700 hidden sm:inline">Math Quest</span>
          </button>
          <button
            onClick={goHome}
            title="Home"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold text-slate-500 bg-slate-100 hover:bg-indigo-100 hover:text-indigo-600 transition-colors"
          >
